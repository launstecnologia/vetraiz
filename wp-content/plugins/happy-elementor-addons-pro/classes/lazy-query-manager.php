...
class Lazy_Query_Manager {
	...
	const QUERY_TEMPLATES = 'templates';
	...
	protected static function get_query_handler() {
		...
		$handlers = [
			...
			self::QUERY_TEMPLATES => 'get_templates',
		];
	...
	public static function get_templates() {
		$search_term = self::get_search_term();
		$template_type = isset($_REQUEST['template_type'])? sanitize_text_field($_REQUEST['template_type']): '';
		...
		if( 'ha_library' == $args['post_type'] && ! empty( $template_type ) ) {
			$args['meta_query'] = [
				[
