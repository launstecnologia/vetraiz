...
function ha_metro_grid_layout_image_list( array $list ) {
	if ( empty( $list ) ) {
		return;
	}

	$dir        = HAPPY_ADDONS_PRO_ASSETS . 'imgs/layout/';
	$extension  = '.svg';
	$laout_list = [];
 ...
 * @return mixed
 */
function ha_metro_grid_video_thumb( $source, $url ) {
	$vid_thumb     = '';
	$default_thumb = Elementor\Utils::get_placeholder_image_src();

	switch ( $source ) {
		case 'youtube':
			$parse_url = wp_parse_url( $url );
			$vid_id = '';
			if ( 'youtu.be' == $parse_url['host'] ) {
				$vid_id = isset( $parse_url['path'] ) ? str_replace( '/', '', $parse_url['path'] ) : '';
			} else {
				wp_parse_str( wp_parse_url( $url, PHP_URL_QUERY ), $url_params );
				$vid_id = isset( $url_params['v'] ) ? $url_params['v'] : '';
			}
			$vid_thumb_get = isset( $vid_id ) ? 'https://img.youtube.com/vi/' . $vid_id . '/maxresdefault.jpg' : $default_thumb;
			if ( ha_metro_grid_check_url_exists( $vid_thumb_get ) ) {
				$vid_thumb = $vid_thumb_get;
			} else {
				$vid_thumb = 'https://img.youtube.com/vi/' . $vid_id . '/0.jpg';
			}
			break;
		case 'vimeo':
			$parse_url = wp_parse_url( $url );
			$vid = isset( $parse_url['path'] ) ? str_replace( '/', '', $parse_url['path'] ) : '';
			$vid = ( $vid && strlen( $vid ) == 9 ) ? $vid : '';
			if ( $vid ) {
				$vid = '235215203';
				$data = wp_remote_get( "http://vimeo.com/api/v2/video/$vid.php" );
				$body = is_serialized($data['body']) ? unserialize( $data['body'] ) : false;
				$response = $data['response'];
				$code = $response['code'];  //200
				$hash = ( 200 == $code && is_array( $body ) ) ? $body :[];
				$thumb     = isset( $hash[0]['thumbnail_large'] ) ? $hash[0]['thumbnail_large'] : $hash[0]['thumbnail'];
				$full_url  = substr( $thumb, 0, -4 );
				$vid_thumb = ha_metro_grid_check_url_exists( $full_url ) ? $full_url : $default_thumb;
			} else {
 ...
 * @return mixed
 */
function ha_metro_grid_check_url_exists( $url ) {
